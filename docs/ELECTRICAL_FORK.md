...
1. **Cheap first pass (ships now)**: raceway run as a LINEAR condition (`EMT-BRANCH`, already seeded in Wave 1) — trace by hand, set the condition's `×N multiplier` to the circuit's conductor count, and the report's LF and every linear-basis material line (conductor footage, coupling count) scale automatically. No new tool.
2. **Real pass (later)**: a new MCP tool (`trace_circuit`?) deriving routing + homerun length from committed device shapes + a panel location — closer in spirit to `derive_transitions` (derives a new quantity from shapes already traced) than anything else in the engine. Read `web/src/lib/transitions.ts` first — it's the closest precedent, including the withheld/refused pattern (`docs/AGENT_GUIDE.md` doctrine) for not shipping a wrong circuit guess as a confident number.
